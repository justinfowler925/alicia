...
Install: `bash scripts/install-alicia.sh`  
...
**Cursor runner:** enabled — `CURSOR_API_KEY` soft-loaded by `alicia-serve.sh` from 1Password Atlas (`op://Atlas/CURSOR_API_KEY/Api key`). Allowlist: `atlas6` + `alicia` only; empty/`sfdc` hints are skipped, never defaulted.
...
Vercel's env API. Code: `alicia/avatars.py`, routes `/api/avatar*`.
...
Plan SSOT: `~/fowler-brain/strategy/plans/operator/2026-07-29-alicia-front-door.md`.
...
`repo_hint=alicia`). `ask_claude` → Anthropic Messages API (`claude.py`; key from
`ANTHROPIC_API_KEY`, soft-loaded by `alicia-serve.sh`). `ask_atlas6` slimmed for
...
Transport is Alicia `/api/session/*` — never Anam. Snap key is
`aliciaMobileResumeSnapshot` so it cannot collide with the FNOL demo.
**Never write back** to `public/demos/fnol/`. Assets: `alicia/static/mobile.*`.

## Zoom AI Companion → Inbox (2026-08-11)

Justin's own meetings now reach the pad. There were already three Zoom pieces —
`scripts/feed_zoom_to_alicia_notes.py`, its wrapper, and
`com.clearspeed.alicia-zoom-notes` — and they work, but they read Salesforce
...
* `alicia/zoom_api.py` — Server-to-Server OAuth. The credentials already existed
...
* `alicia/zoom_ingest.py` — extraction, dedupe, ledger. Reads both the REST
