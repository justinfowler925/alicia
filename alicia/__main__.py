"""Alicia CLI — laptop remote control for Studio Atlas6."""
...
def _die(msg: str, code: int = 1) -> None:
    print(f"alicia: {msg}", file=sys.stderr)
...
def _resolve_thread_id(client: AtlasClient, target: str) -> str:
    ...
    if not match:
        _die(f"no open thread for {target.upper()} — try alicia register … --id {target.upper()}")
    return str(match["id"])
...
def main() -> None:
    ...
    sub.add_parser("mcp", help="Run Alicia MCP server (stdio) for Cursor")
    sub.add_parser("serve", help="Start laptop Alicia UI/API (default :8768)")
    ...
    canon.add_argument(
        "--db",
        dest="canon_db",
        default=os.environ.get("ALICIA_CANON_DB_PATH") or str(canon_db_path()),
        help="Canon SQLite path (default: ~/.alicia/state/canon.sqlite; ALICIA_CANON_DB_PATH overrides)",
    )
    ...
    canon_decision_create.add_argument("--decided-by", default=os.environ.get("ALICIA_OWNER_IDENTITY", "justin.fowler@clearspeed.com"))
    ...
    canon_evidence_verify.add_argument(
        "--verifier",
        default=os.environ.get("ALICIA_OWNER_IDENTITY", "justin.fowler@clearspeed.com"),
    )
    ...
    workflow_policy.add_argument("--profile", default=None, help="Explicit schema2 delivery profile")
    ...
    workflow_receipt.add_argument("--captured-by", default="owner-local-verifier")
    workflow_receipt.add_argument("--captured-by-kind", choices=["human", "worker"], default="human")
    ...
    try:
        ...
        if args.cmd == "health":
            ...
            memory.save_conversation(
                args.message,
                reply,
                title=(args.message or "Alicia chat")[:80],
            )
            print(reply)
            if out.get("atlas6_unreachable"):
                print("\n[alicia] Studio unreachable.", file=sys.stderr)
