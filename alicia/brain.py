...
log = logging.getLogger("alicia.brain")
...
def pack_messages(messages: list[dict[str, str]]) -> tuple[str, str]:
    ...
    for raw in messages or []:
        ...
        if role == "system":
            ...
            continue
        label = "Justin" if role == "user" else "Alicia"
...
def _call(cfg: AliciaCfg, backend: str, system: str, body: str) -> dict[str, Any]:
    if backend != "cursor":
    ...
    root = cfg.cursor_runner.reasoning_root if cfg.cursor_runner else home_path("~/.alicia/app")
...
def _create(cfg: AliciaCfg, **kwargs: Any) -> Any:
...
_RECALL_TOOL = {
    "name": "recall",
    "description": (
        "Search Alicia's laptop memory — working notes and lesson drafts — for "
...
def brain_reply(
    ...
    *,
    ...
    protocol = (
        "\n\nTOOL PROTOCOL (when you need a Alicia tool):\n"
...
def _create_cli(cfg: AliciaCfg, **kwargs: Any) -> Any:
