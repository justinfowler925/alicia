"""Claude Code CLI backend for Alicia completion-only turns.
...
Alicia remains the only process allowed to execute Alicia tools or mutations.
"""
...
from .config import AliciaCfg, ClaudeCfg
...
def ask_claude(
    ...
    *,
    ...
    claude = cfg.claude if isinstance(cfg, AliciaCfg) else cfg
    ...
    sys_prompt = (system or "").strip() or (
        "You are helping Justin via Alicia. Be concise and concrete. "
