...
# "hey rewind" is a Cursor codeword (alicia-review skill). Said here it used to
...
class ConversationManager:
        ...
        *,
        ...
        if channel == "voice" and resilience.voice_killed():
            return TurnResult(
                session_id,
                "fast",
                "Voice is paused (`~/.alicia/state/voice.off`). Type instead, or remove that file.",
                "",
                0,
                error="voice_killed",
            )
        ...
        # lands and supersedes stale in-flight work, but Alicia says nothing.
    ...
    def _land_brain(
        ...
        landed = self.store.append_turn(session_id, "alicia", reply, meta=turn_meta)
    ...
    def _land(
        ...
        *,
        ...
        landed = self.store.append_turn(
            session_id, "alicia", reply, meta=meta
        )
