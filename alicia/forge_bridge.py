...
def handle(request, agent=None, state=None, upload_chunks=None):
    ...
    state = state or Path.home() / ".local/share/alicia-forge-chat"
    ...
    try:
        ...
        if action == "upload":
            ...
            workspace = agent.STATE / "workspaces" / "alicia-chat" / thread_id
        ...
        if action == "send":
            ...
            if old:
                ...
                work_item = "alicia-forge-chat:" + message_id
                with agent.db() as runs:
                    existing = runs.execute("SELECT * FROM runs WHERE work_item=? AND agent='forge'", (work_item,)).fetchone()
                prompt = ("You are chatting directly with Justin through Alicia. Answer his latest message. "
