"""Compile gated Alicia proposals into concrete, reviewable contracts."""
...
def compile_proposal(tool: str, args: dict[str, Any]) -> IntentContract:
    ...
    if tool == "organize_agent_thread":
        ...
        return IntentContract(
            outcome=f"update Alicia organization fields for {agent_id}: {', '.join(changed)}",
    ...
    if tool == "organize_project":
        ...
        return IntentContract(
            outcome=f"update Alicia organization fields for {project_id}: {', '.join(changed)}",
