"""LiveKit voice worker that keeps Alicia's canonical conversation manager as the brain."""
...
log = logging.getLogger("alicia.livekit")
ALICIA_URL = os.environ.get("ALICIA_URL", "http://127.0.0.1:8768")
...
def session_id_from_room(room_name: str) -> str:
    match = re.fullmatch(r"alicia-([0-9a-f]{12})-[0-9a-f]{8}", room_name)
    if not match:
        raise ValueError(f"invalid Alicia voice room: {room_name}")
    return match.group(1)


class AliciaVoiceAgent(Agent):
    def __init__(self, session_id: str, gate: OwnerVoiceGate) -> None:
        super().__init__(instructions="Alicia voice transport; the canonical manager supplies every reply.")
    ...
    async def _reply(self, message: str, *, owner_verified: bool) -> str:
        async with httpx.AsyncClient(timeout=150.0) as client:
            response = await client.post(
                f"{ALICIA_URL}/api/session/{self.session_id}/say",
                json={
                    "message": message,
                    "channel": "voice",
                    "read_only": False,
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        """Hand the finalized transcript to Alicia and schedule its reply directly.
...
class CanonicalBrainMarker(llm.LLM):
    """Makes LiveKit schedule LLM turns; AliciaVoiceAgent.llm_node owns generation."""

    @property
    def model(self) -> str:
        return "alicia-canonical-manager"
    ...
    def chat(self, **kwargs):
        raise RuntimeError("canonical replies must pass through AliciaVoiceAgent.llm_node")
...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    gate = OwnerVoiceGate()
    agent = AliciaVoiceAgent(session_id, gate)
...
def main() -> None:
    ...
    agents.cli.run_app(
        agents.WorkerOptions(
            ...
            host=os.environ.get("ALICIA_VOICE_HEALTH_HOST", "127.0.0.1"),
            port=int(os.environ.get("ALICIA_VOICE_HEALTH_PORT", "8096")),
        )
    )
