"""Alicia MCP server — Cursor talks to Studio Atlas6 via Alicia (laptop client).
...
def _chat(message: str, *, mode: str = "manager", read_only: bool = False) -> str:
    ...
    memory.save_conversation(
        message,
        str(reply),
        title=(message or "Alicia chat")[:80],
    )
    return str(reply)
...
def build_mcp():
    ...
    mcp = FastMCP(
        "alicia",
        instructions=(
            "Alicia is Justin's MacBook client for Studio Atlas. "
    ...
    @mcp.tool()
    def alicia_query(message: str, mode: str = "manager") -> str:
        ...
        For actions that mutate state, use alicia_approve, alicia_dispatch,
        alicia_register, or alicia_chat (with explicit intent).
        """
        return _chat(message, mode=mode, read_only=True)

    @mcp.tool()
    def alicia_explain(target: str) -> str:
    ...
    def alicia_listen(
        ...
        """Record microphone audio, transcribe it with local Whisper, and send it to Alicia.

        read_only=True (recommended) routes the transcription through alicia_query so it
    ...
    @mcp.tool()
    def alicia_speak(text: str) -> str:
        """Speak a Alicia response using ElevenLabs TTS. Returns the path to the generated audio file.
        ...
        try:
            ...
            out_path = state_path("alicia_speak.mp3")
    ...
    @mcp.tool()
    def alicia_chat(message: str, mode: str = "manager") -> str:
        ...
        For read-only status, use alicia_status, alicia_digest, alicia_threads,
        alicia_query, or alicia_explain.
        """
        return _chat(message, mode=mode, read_only=False)

    @mcp.tool()
    def alicia_dispatch(dry_run: bool = True, ingest_linear: bool = False) -> str:
    ...
    @mcp.tool()
    def alicia_peek_slack(limit: int = 10) -> str:
    ...
    @mcp.tool()
    def alicia_approve(target: str, reject: bool = False) -> str:
    ...
    @mcp.tool()
    def alicia_work_status(work_item_id: str) -> str:
    ...
    @mcp.tool()
    def alicia_feedback_batch(reports_json: str) -> str:
    ...
    @mcp.tool()
    def alicia_workflow_scorecard(days: int = 7) -> str:
