...
Alicia could show you thirty running agent threads, twenty scheduled jobs and
...
Three things move here, and each has a different safe way to be touched:
  ...
  Alicia's own services include the one answering this request. Restarting the
...
# allowlist from `__file__` worked in a checkout, where alicia/ sits beside
...
LAUNCH_PREFIX = "com.clearspeed.alicia"
...
# --- Alicia's own launchd services -----------------------------------------


def known_service_labels() -> list[str]:
    """Every Alicia service, whether installed on this machine or shipped here.
...
def list_services() -> list[dict[str, Any]]:
    """Every Alicia service, whether it is loaded, and whether it is up."""
    services = []
...
def service_action(label: str, action: str) -> dict[str, Any]:
    ...
    if label not in known_service_labels():
        raise ControlError(f"{label} is not a Alicia service")
...
def _defer_core_action(label: str, action: str) -> dict[str, Any]:
    ...
    return {
        ...
        "action": action,
        "deferred": True,
        "detail": "Alicia is restarting itself — this connection will drop for a few seconds.",
    }
...
def cancel_agent(agent_id: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pid = int(pid)
    if pid == os.getpid():
        raise ControlError("That pid is Alicia itself")
