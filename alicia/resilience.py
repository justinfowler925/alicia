...
# Voice ask_alicia retries before we let ConvAI speak anything.
...
def kill_file_on(name: str) -> bool:
    """True when ~/.alicia/state/<name> exists."""
...
def ensure_api_killed_by_default() -> None:
    ...
    if not KILL_API.exists():
        KILL_API.write_text(
            "Anthropic Messages API disabled by Alicia resilience policy.\n"
