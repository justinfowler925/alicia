"""Alicia laptop HTTP face — ops board + chat + watchdog."""
...
from .config import AliciaCfg, load_config
...
log = logging.getLogger("alicia.server")
...
class ResetAttemptsBody(BaseModel):
    ...
    reason: str = "alicia operator uncap"
    resume: bool = True
...
def _board_payload(client: AtlasClient, cfg: AliciaCfg) -> dict[str, Any]:
...
def create_app(cfg: AliciaCfg | None = None, *, start_watchdog: bool = True) -> FastAPI:
    ...
    async def _supervisor_loop(app: FastAPI) -> None:
        ...
        while True:
            try:
                ...
                raise
            except Exception as exc:  # noqa: BLE001 — monitoring cannot kill Alicia
    ...
    async def zoom_oauth_callback(code: str = "", state: str = "", error: str = "") -> HTMLResponse:
        ...
        return HTMLResponse(
            "<!doctype html><title>Alicia My Notes connected</title>"
            "<main style='font:16px system-ui;max-width:42rem;margin:5rem auto'>"
            "<h1>Alicia My Notes is connected</h1>"
    ...
    async def atlas5_tab(request: Request) -> dict[str, Any]:
        """Compatibility endpoint; standalone Alicia never probes Atlas5."""
                ...
                for p in paths:
                    results.append(
                        c.frontier_apply(
                            ...
                            notes=req.notes or "batch from Alicia focus",
                        )
                    )
            elif thread_ids:
    ...
    async def cursor_run(request: Request) -> dict[str, Any]:
        ...
        anything (see alicia/watchdog.py), and the cursor runner was NOT moved
    ...
    async def session_convai_token(session_id: str, request: Request) -> dict[str, Any]:
        ...
        prompt = (
            "You are Alicia — Justin's right hand on his laptop for Clearspeed RevOps. "
            ...
            "approvals, or emails. The laptop client owns every answer via ask_alicia "
            "and product TTS. If you must call a tool, call ask_alicia with the user's "
            "exact question and speak nothing else."
        )
        # Same voice as /api/speak — one Alicia, never William's Nucleus voice.
        ...
        overrides: dict[str, Any] = {
            "agent": {
                ...
                "firstMessage": "I'm Alicia. What needs you?",
            },
    ...
    async def convai_custom_llm(request: Request) -> StreamingResponse:
        ...
        expected = (
            ...
            or (os.environ.get("ALICIA_CUSTOM_LLM_SECRET") or "").strip()
        )
        ...
        model = str(body.get("model") or "alicia-product-brain")
    ...
    async def session_voice_token(session_id: str, request: Request) -> dict[str, Any]:
        ...
        room = f"alicia-{session_id}-{nonce}"
    ...
    async def chat(req: ChatRequest, request: Request) -> dict[str, Any]:
        try:
            ...
            log.exception("alicia chat failed")
    ...
    async def transcribe(file: UploadFile = File(...)) -> dict[str, Any]:
        ...
        try:
            ...
            log.exception("alicia transcribe failed")
            raise HTTPException(status_code=500, detail=str(exc)) from exc
        finally:
    ...
    async def speak(req: SpeakBody) -> Response:
        ...
        try:
            ...
            log.exception("alicia speak failed")
            raise HTTPException(status_code=502, detail=str(exc)) from exc
...
def serve(cfg: AliciaCfg | None = None) -> None:
    cfg = cfg or load_config()
