...
(() => {
  ...
  let thread = localStorage.getItem('alicia.forge.thread') || '';
  let pending = JSON.parse(localStorage.getItem('alicia.forge.pending') || 'null');
  ...
  async function api(action, extra = {}) {
    const response = await fetch('/api/forge/request', {
      ...
      method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Alicia-Chat': 'forge' },
  ...
  async function sendPending() {
    ...
    try { data = await api('send', pending); }
      ...
      if (error.status === 409 || error.status === 422) {
        ...
        pending = null; localStorage.removeItem('alicia.forge.pending');
      }
      throw error;
    }
    thread = pending.thread_id; localStorage.setItem('alicia.forge.thread', thread);
    pending = null; localStorage.removeItem('alicia.forge.pending');
  ...
  function open() {
    ...
    $('forge-toggle').textContent = active ? 'Back to Alicia' : 'Forge chat';
    $('forge-toggle').setAttribute('aria-pressed', String(active));
    generation++; clearTimeout(timer);
    if (active) connect();
  }
  function saveAttachments() {
    if (thread) localStorage.setItem('alicia.forge.files.' + thread, JSON.stringify(attachments.map(({file, ...a}) => a)));
  }
  function loadAttachments() {
    attachments = thread ? JSON.parse(localStorage.getItem('alicia.forge.files.' + thread) || '[]') : [];
  ...
  async function ensureThread() {
    ...
    localStorage.setItem('alicia.forge.thread', thread);
  }
      ...
      for (const a of files) {
        ...
        try {
          const response = await fetch(`/api/forge/upload/${thread}/${a.id}?name=${encodeURIComponent(a.name)}`, {
            method: 'POST', headers: {'X-Alicia-Chat': 'forge', 'Content-Type': 'application/octet-stream'}, body: a.file,
          });
          const data = await response.json();
  ...
  $('forge-new').addEventListener('click', async () => {
    ...
    localStorage.removeItem('alicia.forge.thread'); $('forge-threads').value = '';
  ...
  $('forge-threads').addEventListener('change', async () => {
    ...
    localStorage.setItem('alicia.forge.thread', thread); await connect();
  });
  $('forge-composer').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      localStorage.setItem('alicia.forge.pending', JSON.stringify(pending));
      await sendPending();
    } catch (error) { failure(error); }
    finally { busy = false; controls(); schedule(); }
  });
