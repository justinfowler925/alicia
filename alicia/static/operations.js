 ...
 * scheduled jobs with a next-run time, and eight of Alicia's own services,
        ...
        list.push({
          ...
          confirm: {
            ...
            consequence: row.isCore
              ? "Alicia stops answering entirely. You will need a terminal to start it again."
...
function selectTab(id) {
  ...
  try {
    localStorage.setItem("alicia.tray.tab", id);
  } catch {
    /* private mode */
  }
...
function initTabs() {
  ...
  try {
    remembered = localStorage.getItem("alicia.tray.tab") || remembered;
  } catch {
    /* private mode */
  }
...
window.aliciaOps = { ops, PANELS, PANEL_BY_KEY, loadPanel, paint, selectTab, relative, statusTone, statusWords };
})();
