...
function initTheme() {
  let cur = "dark";
  try {
    cur = localStorage.getItem("alicia.theme") || "dark";
  } catch {
    /* ignore */
  }
  applyTheme(cur);
...
const ENROLLMENT_PROMPTS = [
  "Alicia, this is my owner voice enrollment sample one.",
  "I use Alicia to move important work forward deliberately.",
...
async function openSession() {
  ...
  if (requested && /^[0-9a-f]{12}$/i.test(requested)) {
    ...
    if (ok) {
      sessionStorage.setItem("alicia.session", requested);
      return hydrate(requested);
    }
  }
  const saved = sessionStorage.getItem("alicia.session");
  if (saved) {
  ...
  sessionStorage.setItem("alicia.session", body.session_id);
...
/* Alicia's system prompt bans headers, tables and rules but explicitly keeps
...
function renderTurn(turn, { animate = true, live = animate } = {}) {
  ...
  el.className = `turn ${turn.role === "user" ? "user" : "alicia"}`;
  el.dataset.turnId = turn.id;

  const who = document.createElement("div");
  who.className = "who";
  who.append(turn.role === "user" ? "You" : "Alicia");
...
/* --- voice in ----------------------------------------------------------- */
/* Grammar over dictation while Alicia is speaking: the synthesised voice can
...
/* Continuous recognition emits finals as scraps ("hey Alicia", "can you tell
...
/* Everything Alicia has said out loud recently.
 *
 * The mute-while-speaking guard was not enough and the transcript proves it:
 * Alicia said "Let me think about that one.", the mic heard ITSELF, the phrase
...
async function startVoice() {
  ...
  try {
    ...
    // echo from Alicia's own TTS, steady background noise, and aggressive gain
    ...
    const conversation = await Conversation.startSession({
      ...
      onMessage: ({ message, source }) => {
        ...
        if (source === "user") {
          ...
          setConversationFilled();
          appendLocalTurn("alicia", message);
          setVoicePhase("speaking");
        }
      },
      onModeChange: ({ mode }) => {
...
function appendLocalTurn(role, text) {
  ...
  article.className = role === "user" ? "turn user" : "turn alicia";
  const strong = document.createElement("strong");
  strong.textContent = role === "user" ? "You" : "Alicia";
  const body = document.createElement("div");
  body.className = "body";
...
function setVoicePhase(phase, detail = "") {
  ...
  if (phase === "speaking") {
    ...
    if (stateLabel) stateLabel.textContent = "Alicia is speaking";
...
function init() {
  ...
  $("#new-session").addEventListener("click", async () => {
    ...
    sessionStorage.removeItem("alicia.session");
    teardownVoice();
    await openSession();
  });
  ...
  openSession().catch(() => {
    ...
    setStatus("Couldn’t connect to Alicia. Reload to try again.");
  });
  initIdeas();
...
// A direct Forge conversation does not need a Alicia voice session, the work
// board or model canaries. Initialize that workspace when it is first shown.
let aliciaSurfaceStarted = false;
function startAliciaSurface() {
  if (location.hash === "#forge" || aliciaSurfaceStarted) return;
  aliciaSurfaceStarted = true;
  init();
}
...
/* Conversation drives navigation. Any ticket id in a turn — yours or Alicia's —
...
function drawIdeas({ flashId = null } = {}) {
  ...
  if (!all.length) {
    renderQueueEmpty({
      ...
      body: "Capture the next thing on your mind. Alicia keeps the original words and drafts a useful title in the background.",
