"""Tool registry for Alicia chat — lookup-first, hallucination-resistant.
...
def _check_email(client: AtlasClient, limit: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "error": str(exc),
            "hint": "Needs Studio GOOGLE_OAUTH_ACCESS_TOKEN; ingest stays separate (alicia ingest-gmail).",
        }
...
def _check_slack(client: AtlasClient, limit: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "error": str(exc),
            "hint": "Needs Studio SLACK_BOT_TOKEN + channel_ids; ingest stays separate (alicia ingest-slack).",
        }
...
def _ask_atlas6(client: AtlasClient, message: str, mode: str = "manager", ticket_id: str = "") -> dict[str, Any]:
    ...
    try:
        raw = client.chat(message, mode=mode, persona="alicia", ticket_id=ticket_id or None)
...
def _ask_frontier(cfg: AliciaCfg, **contract: Any) -> dict[str, Any]:
    import json
...
def _capture_canon_inbox(raw_capture: str, source: str = "alicia:chat") -> dict[str, Any]:
    ...
    *,
    ...
    reg.register(
        Tool(
            name="get_nucleus",
            description=(
                "Read the canonical Nucleus operating graph used by the Alicia screen: "
    ...
    if not read_only:
        ...
        reg.register(
            Tool(
                ...
                fn=lambda **kwargs: _ask_frontier(cfg or AliciaCfg(), **kwargs),
            )
        )
        reg.register(
            Tool(
    ...
    if not read_only:
        ...
        reg.register(
            Tool(
                ...
                description="Save a working note to Alicia's laptop memory. Use this for reminders, half-formed thoughts, or context Justin wants to keep across sessions.",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    reg.register(
        Tool(
            ...
            description="List or search Alicia working notes in laptop memory.",
            parameters={
                "type": "object",
                "properties": {
                    "q": {"type": "string"},
    ...
    reg.register(
        Tool(
            ...
            description="List recent Alicia conversations so Justin can pick up where he left off.",
            parameters={
                "type": "object",
                "properties": {
    ...
    if not read_only:
        reg.register(
            Tool(
                ...
                description=(
                    "Organize one exact Codex, Cursor, or Claude thread in Alicia: pin, archive, "
                    "snooze, label, link a REV ticket, or add notes. This changes only Alicia's "
        ...
        reg.register(
            Tool(
                ...
                description=(
                    "Organize one exact Nucleus project in Alicia: pin or archive it, set its "
                    "objective, or add notes. This changes only Alicia's local overlay and "
            ...
            Tool(
                ...
                parameters={
                    ...
                    "properties": {
                        ...
                        "source": {
                            "type": "string",
                            "description": "where this came from (default alicia:chat)",
                        },
                    },
