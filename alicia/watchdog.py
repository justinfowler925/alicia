...
* The reconcile/dispatch calls deleted here had no equivalent that Atlas lacked.
  ...
  The ``ready > 0`` guard made Alicia do *less*, never more.
...
loop any more — Alicia can be stopped, and Atlas keeps moving.

The cursor runner (``alicia/cursor_runner.py``) is deliberately NOT driven from
...
log = logging.getLogger("alicia.watchdog")

# This actor owns only local health. Remote scheduling is outside Alicia.
...
class Watchdog:
    def __init__(self, cfg: AliciaCfg, client: AtlasClient | None = None) -> None:
        self.cfg = cfg
    ...
    def start(self) -> None:
        ...
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._loop, name="alicia-watchdog", daemon=True)
        self._thread.start()
        with self._lock:
