"""Alexis presentation transport and turn-linked improvement evidence.
...
def feedback_db(request: Request) -> sqlite3.Connection:
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS alexis_feedback (
...
async def avatar_token(session_id: str, request: Request):
    ...
    if not cfg.anam_api_key or not cfg.anam_avatar_id:
        raise HTTPException(503, "Alexis video is not configured. Voice and text remain available.")
    try:
...
def save_feedback(session_id: str, body: Feedback, request: Request):
    ...
    if not turn or turn.role != "brutus" or turn.meta.get("thinking"):
        raise HTTPException(404, "Choose an answered Alexis turn")
    if body.rating == "needs_work" and not body.correction.strip():
        raise HTTPException(422, "Describe what Alexis should do differently")
    cfg = request.app.state.alexis_config
    central_id = turn.meta.get("central_turn_id")
    if not cfg.alexis_brain_url or not central_id:
        raise HTTPException(409, "Choose an answered turn from the shared brain")
    try:
        response = httpx.post(
            cfg.alexis_brain_url.rstrip("/") + "/v1/feedback",
    ...
    try:
        with conn:
            conn.execute("""INSERT INTO alexis_feedback VALUES (?, ?, ?, ?, ?, ?, ?)
...
def read_feedback(session_id: str, request: Request):
    ...
    try:
        rows = [dict(r) for r in conn.execute(
            "SELECT * FROM alexis_feedback WHERE session_id=? ORDER BY captured_at DESC", (session_id,))]
    finally:
        conn.close()
