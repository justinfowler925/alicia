...
(() => {
  ...
  const pageMode = document.body?.dataset?.forgePage === 'true';
  const forgeActive = () => pageMode || !$('forge-panel').hidden;
  setInterval(() => { if (forgeActive()) paintActivity(); }, 1000);
  function controls() {
  ...
  function schedule() {
    clearTimeout(timer);
    if (forgeActive() && working) timer = setTimeout(refresh, 3000);
  }
  ...
  document.addEventListener('dragover', event => {
    if (forgeActive() && event.dataTransfer.types.includes('Files')) {
  ...
  document.addEventListener('drop', event => {
    if (!forgeActive() || !event.dataTransfer.types.includes('Files')) return;
    event.preventDefault(); $('forge-panel').classList.remove('forge-dragging'); selectFiles(event.dataTransfer.files);
  });
  const toggle = $('forge-toggle');
  if (toggle) toggle.addEventListener('click', () => { location.hash = $('forge-panel').hidden ? 'forge' : ''; });
  if (!pageMode) window.addEventListener('hashchange', open);
