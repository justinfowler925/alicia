...
let SF={q:'',health:'',sort:'name',dir:'asc',page:1,size:10,cols:{schedule:true,last_success_at:true}}, STselected='';
...
function studioRows(){return orderedGridRows(filterRows(ST.jobs,SF.q,['name','source','id','scheduler']).filter(j=>!SF.health||j.health===SF.health),SF.sort,SF.dir);}
...
function studioDetail(){const j=ST.jobs.find(j=>j.id===STselected);if(!j)return '';
  ...
  return `<section id="studio-detail" class="nucleus-detail" tabindex="-1" aria-label="Run details"><h2>${esc(j.name)}</h2><p>${esc(j.source)}</p>
...
function studioHtml(){
  ...
  const summary=Object.entries(ST.summary||{}).map(([k,v])=>`<button class="chip" type="button" aria-pressed="${SF.health===k}" onclick="SF.health=SF.health==='${k}'?'':'${k}';SF.page=1;render()">${studioHealth(k)} <strong>${v}</strong></button>`).join('');
  return `<div data-cite="shadcn-queue" data-product-pattern="brutus-record-grid">
    ...
    <p class="dim">${rows.length} matching jobs · Scroll across for schedule and run details.</p><div class="nucleus-grid" data-grid-scroll tabindex="0" aria-label="Studio jobs; scroll horizontally for all columns">
