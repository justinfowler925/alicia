#!/usr/bin/env python3
"""Start an isolated local Alicia development surface or its shared brain.
...
parser.add_argument("--config-dir", type=Path, default=Path.home() / ".brutus/alicia-development")
args = parser.parse_args()
root = args.config_dir.expanduser()
if args.service == "brain":
    os.environ.setdefault("ALICIA_DB", str(root / "brain.sqlite"))
    os.environ.setdefault("ALICIA_SURFACE_TOKENS_FILE", str(root / "surfaces.json"))
    # Development-only choice. Cloud deployments set an explicit API provider.
    os.environ.setdefault("ALICIA_MODEL_PROVIDER", "claude-cli")
