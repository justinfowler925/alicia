...
(async()=>{
 ...
 await page.locator('#forge-files').setInputFiles({name:'too-large.txt',mimeType:'text/plain',buffer:Buffer.alloc(10*1024*1024+1)});
 await page.getByText('Files must be 10 MB or smaller. Select smaller files.',{exact:true}).waitFor();
