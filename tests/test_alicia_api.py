...
@pytest.fixture
def client(tmp_path):
    cfg = BrutusCfg(voice=VoiceCfg(enabled=True, anam_api_key="private-provider-key", anam_avatar_id="alicia-avatar"),
                    alicia_brain_url="https://brain.example", alicia_brain_token="private-surface-key")
...
def test_homepage_is_preserved_and_pages_have_separate_navigation(client):
    ...
    assert 'id="alicia-frame"' not in home
    assert '/static/alicia.js' not in home
    assert 'id="supervisor-agents"' in home
    assert 'href="/"' in home and 'href="/atlas"' in home and 'href="/forge"' in home
    assert 'id="alicia-frame"' in client.get("/").text
    assert 'id="alicia-frame"' in client.get("/alicia").text
...
def test_avatar_token_is_audio_only_and_does_not_expose_credentials(client):
    ...
    with patch("brutus.alicia.httpx.AsyncClient.post", new=AsyncMock(return_value=response)) as post:
        result = client.post(f"/api/session/{sid}/alicia-token")
...
def test_cross_origin_cannot_start_avatar_or_save_feedback(client):
    assert client.post("/api/session/no/alicia-token", headers={"Origin": "https://attacker.example"}).status_code == 403
...
def test_feedback_central_write_precedes_local_ack(client):
    ...
    with patch("brutus.alicia.httpx.post", side_effect=httpx.ConnectError("offline")):
    ...
    with patch("brutus.alicia.httpx.post", return_value=response) as post:
...
def test_avatar_provisioning_failure_is_actionable_without_leaking_provider_body(client, status, message):
    ...
    with patch("brutus.alicia.httpx.AsyncClient.post", new=AsyncMock(return_value=response)):
        result = client.post(f"/api/session/{sid}/alicia-token")
