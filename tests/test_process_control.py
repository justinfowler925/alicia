...
Alicia could show thirty running agent threads and twenty scheduled jobs and
...
from alicia import process_control as pc
...
def test_only_alicia_services_can_be_addressed():
    labels = pc.known_service_labels()
    assert "com.clearspeed.alicia" in labels
    assert all(label.startswith("com.clearspeed.alicia") for label in labels)

    with pytest.raises(pc.ControlError, match="not a Alicia service"):
...
def test_an_unknown_action_is_refused_before_anything_runs():
    with (
        patch("alicia.process_control.subprocess.run") as run,
        pytest.raises(pc.ControlError, match="Unknown service action"),
    ):
        pc.service_action("com.clearspeed.alicia-livekit-agent", "obliterate")
    assert not run.called


def test_restarting_a_worker_service_kickstarts_it_in_place():
    with patch("alicia.process_control.subprocess.run", return_value=_completed()) as run:
        result = pc.service_action("com.clearspeed.alicia-livekit-agent", "restart")
...
def test_restarting_the_core_waits_for_its_own_response_to_land():
    ...
    """
    with patch("alicia.process_control.subprocess.Popen") as popen:
...
def test_cancelling_an_agent_signals_it_and_waits_before_killing():
    with patch("alicia.process_control._command_of", side_effect=["claude --resume", ""]), \
         patch("alicia.process_control.os.kill") as kill:
...
def test_an_agent_that_ignores_sigterm_is_killed():
    with patch("alicia.process_control._command_of", return_value="claude --resume"), \
         patch("alicia.process_control.os.kill") as kill, \
         patch("alicia.process_control.time.monotonic", side_effect=[0.0, 99.0]), \
         patch("alicia.process_control.time.sleep"):
        result = pc.cancel_agent("claude:abc", [AGENT_ROW])
...
def test_a_recycled_pid_is_refused_rather_than_signalled():
    ...
    with (
        patch("alicia.process_control._command_of", return_value="/Applications/Xcode.app/xcode"),
        patch("alicia.process_control.os.kill") as kill,
...
def test_an_already_exited_thread_is_a_success_not_an_error():
    with patch("alicia.process_control._command_of", return_value=""), \
         patch("alicia.process_control.os.kill") as kill:
        result = pc.cancel_agent("claude:abc", [AGENT_ROW])
...
def test_alicia_refuses_to_signal_itself():
    row = {"id": "claude:self", "pid": os.getpid()}
    with pytest.raises(pc.ControlError, match="Alicia itself"):
...
def test_a_failing_studio_command_surfaces_its_own_message():
    with (
        patch(
            "alicia.process_control.subprocess.run",
...
def _app(tmp_path):
    ...
    with (
        patch("alicia.server.MemoryStore", return_value=memory),
        patch("alicia.conversation.MemoryStore", return_value=memory),
        patch("alicia.server.TodoStore", return_value=todos),
        patch("alicia.conversation.TodoStore", return_value=todos),
        patch("alicia.server.SessionStore", return_value=SessionStore(tmp_path / "sessions.sqlite")),
        patch("alicia.server.AtlasClient") as atlas,
    ):
        atlas.return_value = MagicMock()
        return create_app(AliciaCfg(watchdog_enabled=False), start_watchdog=False)


def test_the_services_endpoint_reports_every_alicia_service(tmp_path, monkeypatch):
...
def test_controlling_a_service_that_is_not_ours_is_a_400(tmp_path):
    ...
    assert "not a Alicia service" in response.json()["detail"]


def test_cancelling_a_thread_the_board_does_not_have_is_a_400(tmp_path):
    from fastapi.testclient import TestClient

    with patch("alicia.server.scan_agent_sessions", return_value=[]):
        client = TestClient(_app(tmp_path))
...
def test_the_service_list_does_not_depend_on_how_alicia_was_installed():
    ...
    assert pc.LAUNCH_PREFIX == "com.clearspeed.alicia"
    # Whatever this machine looks like, the core service is addressable.
    assert "com.clearspeed.alicia" in pc.known_service_labels()
